...
require_once '../config.php';
    class reclamationR {
        ...
        public function getRecByidadmin($id) {
            try {
                $pdo = getConnexion();
                $query = $pdo->prepare(
                    'SELECT * FROM reclamation WHERE idadmin = :id'
                );
                $query->execute([
                    'id' => $id
                ]);
...
public function getRecBysujet($sujett) {
        ...
        public function addRec($reclamation) {
            try {
                $pdo = getConnexion();
                $query = $pdo->prepare(
                    'INSERT INTO reclamation (idRec, iduser, idadmin, sujet, message, file) 
                VALUES (:idRec, :iduser, :idadmin, :sujet, :message, :file)'
                );
                $query->execute([
                    'idRec' => $reclamation->getidRec(),
                    'iduser' => $reclamation->getiduser(),
                    'idadmin' => $reclamation->getidadmin(),
